...
When such an investigation finds that $r \gg 2$ is required, as we have found in different scientific applications, it would be useful to provide some guidance on whether DFR is still feasible or whether alternatives need to be considered.
...
\begin{figure}
    ...
    \includegraphics[width=0.65\linewidth]{figures/bottom-row-plot.pdf}
    \caption{
    \small
...
The CLaRe framework assumes both an encoder and a decoder in order to assess reconstruction error.
Manifold learning and local \Frechet regression act as encoder and decoder analogues, respectively, but not as a conventional reconstruction pair.
This yields a model in which errors appear both in the latent and the object space. One of the attractive features of DFR is that it only requires the response objects to live in a space equipped with a metric.
...
Employing a transformation that decodes objects allows us to eliminate, reduce, or at least diagnose approximation error in the latent space due to object reconstruction, isolating error purely from the latent model.
DFR has approximation error from both sources, and although it is accounted for in the authors' theory, it can be challenging to assess in practice, as highlighted our example above. 
Our framework applied to images \parencite{gunning2026statistical} and distributions \parencite{stanley_2026_representation} employs near-lossless encoder-decoder architectures and parametric latent regression models.
...
In contrast, subsequent developments to the \Frechet regression framework eliminate the dual error source by
learning the regression weights directly using deep learning \parencite{zhou_end--end_2026}. 
These still require a case-by-case instantiation of a local search, usually harnessing additional vectorization that would also admit a decoder architecture.