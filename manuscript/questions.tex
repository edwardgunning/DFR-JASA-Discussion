We appreciate the generality of the mathematical results that this approach allows. In this, case that generality also leaves a significant amount of detail that must be specified for any particular study. 
...
The third row displays the first ten eigenvalues from the MDS step in ISOMAP; we interpret these as an aggregated measure of how well an $r$-dimensional ISOMAP representation preserves the geodesic distances in the dataset.
More in line with our CLaRe philosophy, in the bottom row (left) we present a measure of how well each of the $500$ observations' geodesic distances with the other objects is preserved, for $r=1, \dots, 4$. 
...
When such an investigation finds that $r \gg 2$ is required, as have in different scientific applications, some guidance on whether DFR is still feasible or whether alternatives need to be considered could be provided.
...
\begin{figure}
    ...
    \textbf{Third row}: The eigenvalues obtained from the MDS step in ISOMAP for each of the examples.
    \textbf{Fourth row (bottom panel)}: \textbf{Left}: Boxplot of the individual observations' geodesic distance reconstructions as a function of the ISOMAP dimension $r$, stratified by the binary covariate $X_9$.
...
The CLaRe framework assumes both an encoder and a decoder in order to asssess reconstruction error. Local \Frechet regression maps to an encoder, but not in reverse. This yields a model in which errors appear both in the latent and the object space. One of the attractive features of DFR is that it only requires the response objects to live in a space equipped with a metric.
